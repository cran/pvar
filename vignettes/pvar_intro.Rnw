...
%\VignetteIndexEntry{The breath intro into the package}
%\VignetteDepends{}
...
The core functions of the package is based on algorithm described in \cite{ButkusNorvaisa2014}.
...
It seams that up till \emph{pvar} package
there were no software for $p$-variation calculus (if I am wrong,
please, give me a note).
The package \emph{pvar} is here to fill that hole.
...
More over, the data and supreme partition could be illustrated with \emph{plot} function
<<pvar_plot>>=
plot(pv)
@
...
% Mano straipnis
% \bibitem{ButkusNorvaisa2014} 
% V. Butkus, R. Norvai\v{s}a. The calculus of $p$-variation. Preprint. 
